...
async function initShell({ view } = {}) {
  ...
  const initial = (me.fullName || me.email || "?")
    .trim()
    .charAt(0)
    .toUpperCase();
  ...
  if (nav) {
    nav.innerHTML = NAV_ITEMS.map(
      (item) =>
        `<a class="ec-nav-link${item.view === view ? " active" : ""}" href="${item.href}">${item.label}</a>`,
    ).join("");
  }

  const mustChangeBanner = document.getElementById("mustChangeBanner");
  if (mustChangeBanner)
    mustChangeBanner.classList.toggle("hidden", !me.mustChangePassword);
