...
(async function () {
  ...
  document.getElementById("nr-offer").addEventListener("change", (e) => {
    document
      .getElementById("customOfferField")
      .classList.toggle("hidden", !!e.target.value);
  });

  document
    .getElementById("newReservationForm")
    .addEventListener("submit", async (e) => {
      ...
      const payload = {
        ...
        offerName: offerId
          ? undefined
          : document.getElementById("nr-offerName").value,
...
async function loadOffersIntoSelect() {
  ...
  select.innerHTML =
    '<option value="">Offre sur-mesure</option>' +
    offers.map((o) => `<option value="${o._id}">${o.title}</option>`).join("");
...
async function loadReservations() {
  ...
  `,
      ...
      .join("") ||
    '<div class="empty-note">Vous n\'avez pas encore de réservation. Cliquez sur « Nouvelle réservation » pour commencer, ou parcourez nos <a href="offers.html" style="color:var(--accent);">offres</a>.</div>';

  list
    .querySelectorAll(".res-card")
    .forEach((card) =>
      card.addEventListener("click", () =>
        openReservationDetail(card.dataset.id),
      ),
    );
...
function openReservationDetail(id) {
  ...
  body.querySelector("#sendReplyBtn").addEventListener("click", async () => {
    ...
    if (!text) return;
    const updated = await api(`/reservations/${id}/messages`, {
      method: "POST",
