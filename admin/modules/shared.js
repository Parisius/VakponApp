...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    label: "Réservations",
    group: "Gestion",
    roles: ["admin", "operations", "service_client", "support", "financial"],
  },
  {
    view: "offers",
    href: "offers.html",
  ...
  },
  {
    ...
    label: "Clients (CRM)",
    group: "Gestion",
    roles: ["admin", "operations", "service_client", "support"],
  },
  {
    view: "analytics",
    href: "analytics.html",
...
const NAV_ICONS = {
  dashboard:
    '<svg viewBox="0 0 24 24" fill="none"><rect x="3.5" y="3.5" width="7.5" height="9" rx="1.5" stroke="currentColor" stroke-width="1.7"/><rect x="13" y="3.5" width="7.5" height="5.5" rx="1.5" stroke="currentColor" stroke-width="1.7"/><rect x="13" y="11.5" width="7.5" height="9" rx="1.5" stroke="currentColor" stroke-width="1.7"/><rect x="3.5" y="14.5" width="7.5" height="6" rx="1.5" stroke="currentColor" stroke-width="1.7"/></svg>',
  reservations:
    '<svg viewBox="0 0 24 24" fill="none"><rect x="3.5" y="5" width="17" height="15.5" rx="2.5" stroke="currentColor" stroke-width="1.7"/><path d="M3.5 9.5h17M8 3v3.5M16 3v3.5" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',
  offers:
    '<svg viewBox="0 0 24 24" fill="none"><path d="M11.3 3.5H5a1.5 1.5 0 00-1.5 1.5v6.3c0 .4.16.78.44 1.06l8.9 8.9c.58.58 1.53.58 2.12 0l6.3-6.3a1.5 1.5 0 000-2.12l-8.9-8.9a1.5 1.5 0 00-1.06-.44z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><circle cx="8.2" cy="8.2" r="1.3" fill="currentColor"/></svg>',
  customers:
  ...
  analytics:
    '<svg viewBox="0 0 24 24" fill="none"><path d="M4 20V10M11 20V4M18 20v-7" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  translations:
    '<svg viewBox="0 0 24 24" fill="none"><path d="M4 5h10M9 3v2M9 5c0 4-2.5 7-6 8.5M6 9.5c1.5 2 4 3.5 6.5 4M14 21l4-9 4 9M15.3 18h5.4" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  pages:
    '<svg viewBox="0 0 24 24" fill="none"><path d="M6 3.5h9l3 3V20a1 1 0 01-1 1H6a1 1 0 01-1-1V4.5a1 1 0 011-1z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><path d="M8.5 10h7M8.5 13.5h7M8.5 17h4.5" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>',
};
...
function getIdentity() {
  try {
    return JSON.parse(localStorage.getItem("vakpon_admin_identity") || "null");
  } catch {
    return null;
  }
}
...
function clearIdentity() {
  localStorage.removeItem("vakpon_admin_identity");
}
...
function showToast(
  message,
  { type = "success", persistent = false, copyValue = null } = {},
) {
  ...
  if (copyValue) {
    el.querySelector(".toast-copy").addEventListener("click", async () => {
      try {
        ...
        showToast("Copié dans le presse-papiers.", { type: "success" });
      } catch {
...
function renderPagination(
  ...
  el.querySelectorAll("button[data-page]").forEach((b) =>
    b.addEventListener("click", () => onPageChange(Number(b.dataset.page))),
  );
}
...
async function initShell({ view, requiredRoles = null, onSearch = null } = {}) {
  ...
  try {
    setIdentity(await api("/users/me"));
  } catch {
    window.location.href = "login.html";
    return null;
  }
  ...
  const initial = (identity.fullName || identity.email || "?")
    .trim()
    .charAt(0)
    .toUpperCase();
  ...
  if (sidebarName)
    sidebarName.textContent = identity.fullName || identity.email;
  if (sidebarRole)
    sidebarRole.textContent = ROLE_LABELS[identity.role] || identity.role;
  if (topbarAvatar) topbarAvatar.textContent = initial;
  if (profileMenuName)
    profileMenuName.textContent = identity.fullName || identity.email;
  if (profileMenuEmail) profileMenuEmail.textContent = identity.email || "";

  const nav = document.getElementById("sidebarNav");
  if (nav) {
    const visible = NAV_ITEMS.filter(
      (item) => !item.roles || item.roles.includes(identity.role),
    );
    let html = "";
    let lastGroup = null;
...
function initChangePassword(identity) {
  ...
  if (
    profileMenu &&
    logoutBtn &&
    !document.getElementById("changePasswordMenuBtn")
  ) {
...
function setChangePasswordForced(forced) {
  ...
  modal.querySelector(".modal-backdrop").style.pointerEvents = forced
    ? "none"
    : "";
  modal
    .querySelector("#changePasswordNote")
...
const ACTION_LABELS = {
  "member.create": "Création membre",
  "member.update": "Modification membre",
  "member.remove": "Suppression membre",
  "password.reset": "Réinitialisation mot de passe",
  "password.change": "Changement mot de passe",
  "password.forgot": "Mot de passe oublié",
  "reservation.status": "Statut réservation",
  "reservation.notes": "Notes réservation",
  "customer.update": "Fiche client",
  "offer.create": "Création offre",
  "offer.update": "Modification offre",
  "offer.delete": "Suppression offre",
