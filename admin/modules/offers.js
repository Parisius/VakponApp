...
(async function () {
  ...
  document
    ...
    .addEventListener("click", () => openOfferForm(null));
  document
    .getElementById("exportCsv")
    .addEventListener("click", exportCsvClick);
  document
    .getElementById("exportExcel")
    .addEventListener("click", exportExcelClick);
...
function render() {
  ...
  `,
      ...
      .join("") ||
    `<tr><td colspan="5" style="color:var(--muted);">Aucune offre.</td></tr>`;

  tbody.querySelectorAll("tr[data-id]").forEach((row) => {
    row.addEventListener("click", (e) => {
      if (!e.target.closest("[data-delete],[data-toggle]"))
        openOfferForm(row.dataset.id);
    });
  });
      ...
      if (
        !confirm(
          nextActive
            ...
            : "Marquer cette offre comme expirée ? Elle disparaîtra du site.",
        )
      )
        return;
      try {
        await api(`/admin/offers/${btn.dataset.toggle}`, {
          method: "PATCH",
          body: JSON.stringify({ active: nextActive }),
        });
        load();
        showToast(
          nextActive ? "Offre réactivée." : "Offre marquée comme expirée.",
        );
      } catch (err) {
  ...
  tbody.querySelectorAll("[data-delete]").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      try {
        await api(`/admin/offers/${btn.dataset.delete}`, { method: "DELETE" });
        load();
  ...
  renderPagination("pagination", list.length, page, PAGE_SIZE, (p) => {
    page = p;
    render();
  });
}
...
function openOfferForm(id) {
  ...
  body
    ...
    .forEach((el) =>
      el.addEventListener("click", () => closeModal("offerModal")),
    );
  ...
  function addPricingBreakdownRow(
    label = "",
    labelEn = "",
    amount = "",
    highlight = false,
  ...
  function renumberDays() {
    itineraryList.querySelectorAll(".day-card .day-num").forEach((el, idx) => {
      el.textContent = `Jour ${idx + 1}`;
    });
  }
  ...
  (o.includedItems || []).forEach((i, idx) =>
    addIncludedRow(i, (o.includedItemsEn || [])[idx] || ""),
  );
  (o.priceTiers || []).forEach((p) =>
    addPriceRow(p.label, p.labelEn, p.amount),
  );
  (o.modalPricingBreakdown || []).forEach((p) =>
    addPricingBreakdownRow(p.label, p.labelEn, p.amount, p.highlight),
  );
  (o.itinerary || []).forEach((d) =>
    addItineraryRow(
      d.dateLabel,
      d.dateLabelEn,
      d.title,
  ...
  body
    .querySelector("#addIncluded")
    .addEventListener("click", () => addIncludedRow());
  body
    .querySelector("#addPrice")
    .addEventListener("click", () => addPriceRow());
  body
    .querySelector("#addPricingRow")
    .addEventListener("click", () => addPricingBreakdownRow());
  body
    .querySelector("#addItineraryDay")
    .addEventListener("click", () => addItineraryRow());
  ...
  body.querySelector("#saveOfferBtn").addEventListener("click", async () => {
    ...
    const payload = {
      ...
      includedItems: includedRows
        .map((row) => field(row, "fr"))
        .filter(Boolean),
      includedItemsEn: includedRows
        .map((row) => field(row, "en"))
        .filter(Boolean),
      priceTiers: [...priceList.querySelectorAll(".price-row")]
        .map((row) => ({
          label: field(row, "label"),
          labelEn: field(row, "labelEn"),
      ...
      modalPricingBreakdown: [
        ...pricingBreakdownList.querySelectorAll(".price-row"),
      ]
        .map((row) => ({
          label: field(row, "label"),
          labelEn: field(row, "labelEn"),
          amount: field(row, "amount"),
          highlight: row.querySelector(".pb-highlight").checked,
        }))
        .filter((p) => p.label && p.amount),
      itinerary: [...itineraryList.querySelectorAll(".day-card")]
        .map((row) => ({
          dateLabel: field(row, "dateLabel"),
          dateLabelEn: field(row, "dateLabelEn"),
          title: field(row, "title"),
          titleEn: field(row, "titleEn"),
          description: field(row, "description"),
          descriptionEn: field(row, "descriptionEn"),
        }))
        .filter((d) => d.dateLabel && d.title),
    };
    try {
      if (id)
        await api(`/admin/offers/${id}`, {
          method: "PATCH",
...
function exportCsvClick() {
  const rows = filtered().map((o) => [
    o.title,
    o.slug,
    o.priceTiers?.[0]?.amount || "",
    o.featured ? "Oui" : "Non",
    o.active ? "Oui" : "Non",
  ]);
  exportCsv(
    "offres",
    ["Titre", "Slug", "Prix individuel", "Mise en avant", "Active"],
    rows,
  );
}
