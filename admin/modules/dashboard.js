...
(async function () {
  ...
  const identity = await initShell({
    view: "dashboard",
    requiredRoles: Object.keys(ROLE_LABELS),
  });
  if (!identity) return;
  ...
  if (canSee("reservations")) {
    try {
      reservations = await api("/admin/reservations");
    } catch {
      /* no access */
    }
  }
  ...
  document.getElementById("periodFilter").addEventListener("click", (e) => {
    ...
    document
      .querySelectorAll("#periodFilter [data-period]")
...
function buildBuckets(period, allReservations) {
  ...
  const earliest = allReservations.reduce(
    (min, r) => Math.min(min, new Date(r.createdAt).getTime()),
    Date.now(),
  );
...
function render() {
  ...
  if (canSee("customers")) {
    ...
    const newCustomers = periodStart
      ? customers.filter((c) => new Date(c.createdAt) >= periodStart).length
      : customers.length;
    kpis.push({
      label: `Nouveaux clients · ${periodLabel}`,
      value: newCustomers,
    });
  }
  if (canSee("team")) {
  ...
  `,
      ...
      .join("") ||
    '<div style="color:var(--muted);font-size:14px;">Aucune donnée disponible pour votre rôle.</div>';
  ...
  if (canSee("reservations")) {
    ...
    `,
        ...
        .join("") ||
      '<div style="color:var(--muted);font-size:13.5px;">Aucune réservation.</div>';
  } else {
    chartTitle.textContent = "Réservations";
    chartEl.innerHTML =
      '<div style="color:var(--muted);font-size:13.5px;">Non disponible pour votre rôle.</div>';
  }
  ...
  if (canSee("reservations")) {
    ...
    statusEl.innerHTML = Object.entries(counts)
      .map(
        ([status, count]) => `
  ...
  if (canSee("reservations") && periodReservations.length) {
    ...
    topOffersEl.innerHTML =
      '<div style="color:var(--muted);font-size:13.5px;">Non disponible pour votre rôle.</div>';
  } else {
    topOffersEl.innerHTML =
      '<div style="color:var(--muted);font-size:13.5px;">Aucune réservation sur cette période.</div>';
  }
}
