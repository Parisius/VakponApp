...
function filtered() {
  ...
  return customersCache.filter((c) =>
    [c.fullName, c.email, c.phone, ...(c.tags || [])]
      .filter(Boolean)
      .join(" ")
      .toLowerCase()
      .includes(query),
...
function render() {
  ...
  `,
      ...
      .join("") ||
    `<tr><td colspan="5" style="color:var(--muted);">Aucun client.</td></tr>`;
...
function openCustomer(id) {
  ...
  body.querySelector("#saveCustomerBtn").addEventListener("click", async () => {
    try {
      const tags = body
        .querySelector("#c-tags")
        .value.split(",")
        .map((t) => t.trim())
        .filter(Boolean);
      const adminNotes = body.querySelector("#c-notes").value;
      await api(`/admin/customers/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ tags, adminNotes }),
      });
