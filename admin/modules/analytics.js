...
(async function () {
  const identity = await initShell({
    view: "analytics",
    requiredRoles: ["admin", "operations", "marketing"],
  });
  if (!identity) return;
  ...
  document.getElementById("periodFilter").addEventListener("click", (e) => {
    ...
    document
      .querySelectorAll("#periodFilter [data-range]")
  ...
  document
    ...
    .addEventListener("click", () => exportLog("excel"));
...
function renderLineChart(el, days) {
  if (!days.length) {
    el.innerHTML =
      '<div style="color:var(--muted);font-size:13.5px;">Aucune visite sur cette période.</div>';
    return;
  }
  ...
  const pathsHtml = series
    .map((s) => {
      ...
      const dots = days
        .map(
          (d, i) =>
            `<circle cx="${xAt(i)}" cy="${yAt(d[s.key])}" r="2.5" fill="${s.color}"></circle>`,
        )
        .join("");
...
async function render() {
  ...
  try {
    data = await api(
      `/admin/analytics/summary?range=${currentRange}&path=${encodeURIComponent(currentPath)}&device=${encodeURIComponent(currentDevice)}`,
    );
  } catch {
    showToast("Impossible de charger les statistiques de visite.", {
      type: "error",
    });
    return;
  }
  ...
  const knownPaths = new Set(
    Array.from(pathFilter.options).map((o) => o.value),
  );
  data.topPages.forEach((p) => {
    if (!knownPaths.has(p.path)) {
      pathFilter.insertAdjacentHTML(
        "beforeend",
...
async function renderLog() {
  ...
  try {
    data = await api(
      `/admin/analytics/log?range=${currentRange}&path=${encodeURIComponent(currentPath)}&device=${encodeURIComponent(currentDevice)}&page=${currentLogPage}&limit=${LOG_PAGE_SIZE}`,
    );
  } catch {
    showToast("Impossible de charger le journal des visites.", {
      type: "error",
    });
    return;
  }
  ...
  `,
      ...
      .join("") ||
    `<tr><td colspan="12" style="color:var(--muted);text-align:center;">Aucune visite sur cette période.</td></tr>`;
...
async function exportLog(format) {
  ...
  try {
    data = await api(
      `/admin/analytics/log?range=${currentRange}&path=${encodeURIComponent(currentPath)}&device=${encodeURIComponent(currentDevice)}&page=1&limit=1000`,
    );
  } catch {
    showToast("Impossible d'exporter le journal des visites.", {
      type: "error",
    });
    return;
  }
