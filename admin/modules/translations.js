...
function render(filter) {
  ...
  if (filter) {
    tabsEl.hidden = true;
    rows = allTranslations.filter(
      (r) =>
        r.key.toLowerCase().includes(filter) ||
        r.fr.toLowerCase().includes(filter) ||
  ...
  `,
      ...
      .join("") ||
    `<tr><td colspan="4" style="color:var(--muted);text-align:center;">Aucun résultat.</td></tr>`;
  ...
  tbody.querySelectorAll("[data-save]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      btn.disabled = true;
      try {
        await api("/admin/translations", {
          method: "PATCH",
          body: JSON.stringify({ key, fr, en }),
        });
