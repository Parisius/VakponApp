...
(async function () {
  ...
  if (!identity) return;
  canManage = ["admin", "operations", "service_client", "support"].includes(
    identity.role,
  );
...
async function load() {
  ...
  reservationsCache = await api(
    `/admin/reservations${status ? `?status=${status}` : ""}`,
  );
  render();
}
...
function render() {
  ...
  `,
      ...
      .join("") ||
    `<tr><td colspan="6" style="color:var(--muted);">Aucune réservation.</td></tr>`;

  tbody
    .querySelectorAll("tr[data-id]")
    .forEach((row) =>
      row.addEventListener("click", () => openReservation(row.dataset.id)),
    );
      ...
      <div class="modal-side">
        ...
        <div class="status-row" style="margin:0;">
          ${Object.keys(STATUS_LABELS)
            .map(
              (s) =>
                `<button ${canManage ? "" : "disabled"} class="btn-outline small-btn" data-status="${s}" style="${s === r.status ? "border-color:var(--accent);color:var(--accent);" : ""}">${STATUS_LABELS[s]}</button>`,
            )
            .join("")}
        </div>
      </div>
  ...
  if (canManage) {
    body.querySelectorAll("[data-status]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        try {
          await api(`/admin/reservations/${id}/status`, {
            method: "PATCH",
          ...
          openReservation(id);
          showToast(
            `Statut mis à jour : ${STATUS_LABELS[btn.dataset.status]}.`,
          );
        } catch (err) {
    ...
    body.querySelector("#saveNotesBtn").addEventListener("click", async () => {
      try {
        await api(`/admin/reservations/${id}/notes`, {
          method: "PATCH",
          body: JSON.stringify({
            adminNotes: document.getElementById("adminNotesInput").value,
          }),
        });
    ...
    body
      ...
      .addEventListener("click", async () => {
        try {
          ...
          if (!text) return;
          await api(`/reservations/${id}/messages`, {
            method: "POST",
