...
document
  ...
  .forEach((el) => el.appendChild(logoTpl.content.cloneNode(true)));
document
  .querySelectorAll("[data-logo-white]")
  .forEach((el) => el.appendChild(logoTplWhite.content.cloneNode(true)));
...
window.addEventListener("scroll", () => {
  ...
  if (heroSection)
    heroSection.classList.toggle("scrolled-past", window.scrollY > 40);
});
...
(function initActiveNav() {
  const navLinks = document.querySelectorAll(
    "nav a, .page-nav a, .ff-menu-links a",
  );
  ...
  function setActive(matchHref) {
    navLinks.forEach((a) => {
      ...
      a.classList.toggle(
        "active",
        !a.classList.contains("espace-client-link") &&
          !!matchHref &&
          href === matchHref,
      );
    });
...
function refreshScrollRevealText() {
  ...
  els.forEach((el) => {
    ...
    el.innerHTML = words
      .map((w) => `<span class="sr-word">${w}</span>`)
      .join(" ");
  });
...
const API_BASE = IS_LOCAL
  ...
  : "https://api.vakpon-tours.com/api";
const ESPACE_CLIENT_URL = IS_LOCAL
  ? "http://localhost:5502/index.html"
  : "https://vakpon-tours.com/espace-client/index.html";
document.querySelectorAll(".espace-client-link").forEach((el) => {
  el.href = ESPACE_CLIENT_URL;
});
...
try {
  ...
  /* private browsing etc. - tracking just skips session grouping */
}
...
let currentLang =
  localStorage.getItem("vakpon-lang") ||
...
function pickList(obj, field) {
  if (!obj) return [];
  const enValue = obj[field + "En"];
  return currentLang === "en" && enValue && enValue.length
    ? enValue
    : obj[field] || [];
}
...
function applyTranslations() {
  ...
  document
    .querySelectorAll(".lang-toggle, [data-lang-toggle]")
    .forEach((btn) => {
  ...
  if (typeof startTypewriter === "function" && translations["hero.typewriter"])
    startTypewriter(t("hero.typewriter"));
}
...
fetch(`${API_BASE}/translations?site=vakpon-tours`)
  ...
  .catch(() => {
    /* falls back to the French baked into the HTML */
  });
...
if (contactForm)
  contactForm.addEventListener("submit", async function (e) {
    ...
    const offerName =
      offerSelect.options[offerSelect.selectedIndex]?.textContent || "";
    ...
    const payload = {
      ...
      travelers:
        parseInt(document.getElementById("resTravelers").value, 10) || 1,
    };
    if (offerId) payload.offerId = offerId;
    else payload.offerName = offerName;
...
document.querySelectorAll(".theme-toggle").forEach((btn) => {
  btn.addEventListener("click", (e) => {
    ...
    const apply = () => {
      ...
      localStorage.setItem("vakpon-theme", next);
    };
...
const ffMenuToggles = [
  document.getElementById("ffMenuToggle"),
...
if (ffMenuPanel && ffMenuToggles.length) {
  ...
  ffMenuToggles.forEach((btn) => {
    btn.addEventListener("click", () =>
      setOpen(!ffMenuPanel.classList.contains("open")),
    );
  });
  document.addEventListener("click", (e) => {
    if (
      ffMenuToggles.some((btn) => btn.contains(e.target)) ||
      ffMenuPanel.contains(e.target)
    )
      return;
    closeFfMenu();
  });
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") closeFfMenu();
  });
}
...
if (ffPaginationEl) {
  ...
  const offerTextFallback = {
    welcome: {
      fr: "Offre Spéciale : Places Limitées",
      en: "Special Offer: Limited Spots",
    },
    headline: {
      fr: "Pack Séjour Bénin<br>7 jours / 6 nuits <br> dès 2 232€",
      en: "Benin Stay Package<br>7 days / 6 nights <br> from €2,232",
    },
  };
  ...
  let heroOffersList = [];
  {
    ...
    const initialPins = [
      {
        title: { fr: "Route des Esclaves", en: "Slave Route" },
        sub: { fr: " · Ouidah", en: " · Ouidah" },
      },
      {
        title: { fr: "Collines de Dassa", en: "Dassa Hills" },
        sub: { fr: " · Dassa-Zoumè", en: " · Dassa-Zoumè" },
      },
      {
        title: { fr: "Tata Somba", en: "Tata Somba" },
        sub: { fr: " · Boukoumbé", en: " · Boukoumbé" },
      },
      {
        title: { fr: "Ganvié", en: "Ganvié" },
        sub: { fr: " · Cité lacustre", en: " · Lake Village" },
      },
      {
        title: { fr: "Porto-Novo", en: "Porto-Novo" },
        sub: { fr: " · Capitale politique", en: " · Political Capital" },
      },
      {
        title: { fr: "Statue Bio Guerra", en: "Bio Guerra Statue" },
        sub: { fr: " · Cotonou", en: " · Cotonou" },
      },
    ];
  ...
  function ffSegCount() {
    return ffPaginationEl.querySelectorAll(".seg").length;
  }
  ...
  function setFfSlide(index) {
    ...
    const defaultWelcome = translations["hero.defaultWelcome"]
      ? t("hero.defaultWelcome")
      : "Avec Vakpon Tours";
    const defaultHeadline = translations["hero.defaultHeadline"]
      ? t("hero.defaultHeadline")
      : "Vivez une nouvelle façon<br>de découvrir le Bénin";
    if (ffWelcome)
      ffWelcome.textContent = isOffer
        ? meta.offer
          ? pick(meta.offer, "heroWelcomeText")
          : offerTextFallback.welcome[currentLang]
        : defaultWelcome;
    if (ffHeadline)
      ffHeadline.innerHTML = isOffer
        ? meta.offer
          ? escapeHtml(pick(meta.offer, "heroHeadline")).replace(/\n/g, "<br>")
          : offerTextFallback.headline[currentLang]
        : defaultHeadline;
    if (ffCta) ffCta.classList.toggle("show", isOffer);
    if (meta && ffPin) {
      if (isOffer && meta.offer) {
        pinTitle.textContent =
          pick(meta.offer, "heroPinTitle") || meta.offer.title;
        pinSub.textContent = pick(meta.offer, "heroPinSub")
          ? " · " + pick(meta.offer, "heroPinSub")
          : "";
      } else if (meta.pin) {
  ...
  if (ffSegCount()) {
    ...
    ffPaginationEl.addEventListener("click", (e) => {
      ...
      if (!seg) return;
      const i = Array.from(ffPaginationEl.querySelectorAll(".seg")).indexOf(
        seg,
      );
      if (i === -1) return;
      ffIndex = i;
    ...
    ffPaginationEl.querySelectorAll(".seg").forEach((seg) => {
      seg.style.cursor = "pointer";
    });
  }

  if (ffCta)
    ffCta.addEventListener("click", () => {
      const segs = Array.from(ffPaginationEl.querySelectorAll(".seg"));
  ...
  document.addEventListener("click", (e) => {
    if (e.target.closest("[data-open-offer]")) {
      e.preventDefault();
      openOfferModal();
      return;
    }
    ...
    if (reserveTrigger)
      selectOfferInForm(
        reserveTrigger.getAttribute("data-reserve-offer"),
        reserveTrigger.getAttribute("data-reserve-name"),
      );
    if (e.target.closest("[data-close-offer]")) {
      closeOfferModal();
      return;
    }
  });
  ...
  function renderOfferCard(offer, index) {
    ...
    const included = pickList(offer, "includedItems")
      .map((i) => `<div>${escapeHtml(i)}</div>`)
      .join("");
    const pricing = (offer.priceTiers || [])
      .map(
        (tier) =>
          `<div class="o-price-tier"><span>${escapeHtml(pick(tier, "label"))}</span><b>${escapeHtml(tier.amount)}</b></div>`,
      )
      .join("");
    const metaBits = [
      pick(offer, "durationLabel"),
      pick(offer, "routeLabel"),
    ].filter(Boolean);
    const metaHtml = metaBits.length
      ? `<div class="o-meta"><b>${escapeHtml(metaBits[0])}</b>${metaBits[1] ? " - " + escapeHtml(metaBits[1]) : ""}</div>`
      : "";
    const badge = isSpecial
      ? `<div class="o-badge">${escapeHtml(t("offerCard.specialBadge"))}</div>`
      : "";
    const cta = isSpecial
  ...
  function populateOfferSelect(offers) {
    ...
    select
      .querySelectorAll("option[data-dynamic-offer]")
      .forEach((o) => o.remove());
    const options = offers
      .map(
        (o) =>
          `<option data-dynamic-offer value="${o._id}">${escapeHtml(pick(o, "title"))}</option>`,
      )
      .join("");
  ...
  function populateOfferModal(offer) {
    ...
    if (priceRow && offer.priceTiers && offer.priceTiers.length) {
      priceRow.innerHTML = offer.priceTiers
        .map(
          (tier, i) => `
    ...
    if (included && includedItems.length) {
      included.innerHTML = includedItems
        .map((i) => `<div>${escapeHtml(i)}</div>`)
        .join("");
    }
    ...
    if (
      breakdown &&
      offer.modalPricingBreakdown &&
      offer.modalPricingBreakdown.length
    ) {
      breakdown.innerHTML = offer.modalPricingBreakdown
        .map(
          (r) => `
    ...
    if (itinerary && offer.itinerary && offer.itinerary.length) {
      itinerary.innerHTML = offer.itinerary
        .map(
          (d) => `
  ...
  function renderOffers(offers) {
    ...
    if (stack) {
      stack
        .querySelectorAll(".offer-card:not(.offer-card-sur-mesure)")
